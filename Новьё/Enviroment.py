...
class Enviroment():
    ...
    def step(self, action):
            ...
            for alie in self.alies_RTK_group_sprite.spritedict:
                if not alie.state_life:
                    try:
                        ...
                        color[alie.id][:] = 0
                        #print("X: {0} Y: {1}, Theta: {2}".format(alie.x_pos, alie.y_pos, alie.theta,))
                        #alie.edge_RTK()
                        #time.sleep(60)
        ...
        if self.mode_war:
            for alie in self.alies_RTK_group_sprite.spritedict:
                if (color[alie.id][0], color[alie.id][1], color[alie.id][2]) == (255, 0, 0) and not alie.state_life2:
            ...
            for enem in self.enemy_RTK_group_sprite.spritedict:
                ...
                if (color1[enem.num][0], color1[enem.num][1], color1[enem.num][2]) == (0, 0, 255) and not enem.state_life2:

                    enem.state_life2 = True
                    self.map.fill(self.white)
                    self.obstacle_group_sprite.draw(self.map)
                    #self.alies_RTK_group_sprite.draw(self.map)
                    enem.draw_boom()
